...
train ROC AUC of `{python} round(float(metrics['roc_auc'][0]),3)` and a test ROC AUC of `{python} round(float(metrics['roc_auc'][1]),3)`. 
...
with open("../results/models/logistic_reg.pkl", "rb") as f:
    lr_best_model = pickle.load(f)
...
train_df = pd.read_csv("../data/processed/sepsis_train.csv")
test_df  = pd.read_csv("../data/processed/sepsis_test.csv")

X_train = train_df.drop(columns=["hospital_outcome", "hospital_outcome_cat"])
X_test  = test_df.drop(columns=["hospital_outcome", "hospital_outcome_cat"])

feature_names = lr_best_model.named_steps["columntransformer"].get_feature_names_out()
clean_feature_names = [
    f.replace("onehotencoder__", "")
     .replace("standardscaler__", "")
     .replace("sex_male", "is_male")
     .replace("sex_female", "is_female")
...
shap_importance = (
    pd.DataFrame({"feature": clean_feature_names, "mean_abs_shap": mean_abs_shap})
    .sort_values("mean_abs_shap", ascending=False)
    .reset_index(drop=True)
)

top1   = shap_importance.iloc[0]
top2   = shap_importance.iloc[1]
bottom = shap_importance.iloc[-1]
```

The SHAP results confirm that **`{python} top1['feature']`** is the most influential feature, with a mean SHAP value of **`{python} f"{top1['mean_abs_shap']:.5f}"`**, indicating that it has 
the strongest impact on survival predictions. The next most important feature is **`{python} top2['feature']`**, with a mean SHAP value of **`{python} f"{top2['mean_abs_shap']:.5f}"`**, 
although substantially weaker than **`{python} top1['feature']`**. 
Finally, **`{python} bottom['feature']`** shows the lowest SHAP contribution, at **`{python} f"{bottom['mean_abs_shap']:.5f}"`**, suggesting minimal influence on the model output.
...
The model achieved a test ROC AUC of **`{python} f"{metrics['roc_auc'][1]:.3f}"`**, indicating limited discriminative ability.  
